...
// Note: For server-side use, use FIGMA_API_TOKEN (not NEXT_PUBLIC_)
// For client-side use, use NEXT_PUBLIC_FIGMA_API_TOKEN
const FIGMA_API_TOKEN = process.env.FIGMA_API_TOKEN || process.env.NEXT_PUBLIC_FIGMA_API_TOKEN;
const FIGMA_FILE_KEY = process.env.FIGMA_FILE_KEY || process.env.NEXT_PUBLIC_FIGMA_FILE_KEY;
 ...
 */
export function findNodeByName(node: FigmaNode, name: string, caseSensitive: boolean = true): FigmaNode | null {
  ...
  if (node.children) {
    for (const child of node.children) {
      const found = findNodeByName(child, name, caseSensitive);
...
/**
 * Helper: Find node by name (case-insensitive)
 */
export function findNodeByNameCaseInsensitive(node: FigmaNode, name: string): FigmaNode | null {
  return findNodeByName(node, name, false);
}

...
export async function extractColorTokens(
  ...
  const colorsFrame = findNodeByName(file.document, frameName);
...
export async function extractTypographyTokens(
  ...
  const file = await getFigmaFile(fileKey);
  const typographyFrame = findNodeByName(file.document, frameName);
...
export async function extractSpacingTokens(
  ...
  const file = await getFigmaFile(fileKey);
  const spacingFrame = findNodeByName(file.document, frameName);
